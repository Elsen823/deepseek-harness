...
`provider` selects a provider route registered by the chosen Cordis composition; `model` is the model id resolved by that adapter. `max_tokens` is an optional positive per-request output-token cap for the root agent and its in-process descendants; omission leaves the provider default in control. Compaction summaries keep the separate limit configured by their compaction plugin. `driver_id` selects the default immutable Agent Driver for SDK-created Sessions; `harness.session(session_id, driver_id=...)` and `run(..., driver_id=...)` may select one for lazy creation, but an existing Session rejects a different value. `harness.drivers()` reads the active catalog, and `harness.runtime(session_id)` returns the Host process-local availability, activity, operation, and independent approval/question counts without activating the Session. The bundled default composition registers `deepseek-official` and the built-in `dsh` Driver. A custom composition can mount additional model adapters and Agent Drivers.
...
`selected_model_selection(events)` folds the latest durable `model/selected` intent from `RunResult.events`. It keeps accepted provider/model/effort intent separate from effective request evidence in `request/header` or `agent-driver/model-request`, so a selection accepted before its first request remains observable without being treated as a completed model call.

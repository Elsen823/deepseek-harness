...
Testing tiers and rules for meaningful green results. Commands live in root [AGENTS.md](../AGENTS.md); linked Agent Notes carry rationale.

## Tiers

- **Unit** (`pnpm run test`): Vitest runs package/example `tests/**` and repository `scripts/**/*.spec.ts`; tests stay near their code. Every registry tests disposal cleanup. Prefer edge cases, errors, ordering, concurrency, and permanent contract-regression tests (see `packages/core/agent-loop/tests/contract-regressions.spec.ts`).
- **Coverage gate** (`pnpm run test:coverage`): CI requires per-file 100% for `packages/*/*/src`; uncovered lines may be dead code, and line coverage proves execution, not behavior. `packages/shell/pwsh-local/src` needs real `pwsh`; without it tests self-skip and `vitest.config.ts` exempts the file, while CI with `pwsh` enforces 100%.
- **Real-API e2e** (`pnpm run test:e2e`): with-key tests against live provider APIs — the DeepSeek model plus provider-specific smokes that gate on their own keys (`EXA_API_KEY`, `PERPLEXITY_API_KEY`, …); each suite self-skips without its key so keyless CI stays green ([real-API e2e Agent Note](../.agents/notes/implemented/testing/2026-06-19-real-api-e2e-ci.md)).
- **Snapshot** (`pnpm run test:snapshot`): keyless expected outputs cover transport, presentation, and persisted assembled behavior. ACP replays its real automation-server example and diffs normalized JSON-RPC/logs ([ACP snapshot Agent Note](../.agents/notes/implemented/testing/2026-06-19-acp-snapshot-tests.md)); headless boots explicit compositions through its JSONL driver, while `apps/cli` owns `dsh --profile headless` acceptance. Use `record` for model transcript changes and `refresh` for valid replay input; review every diff. `text-turn` pins full system-prompt/tool-schema content; other fixtures tokenize it ([pinned-header Agent Note](../.agents/notes/archived/testing/2026-07-06-pin-request-header-content-in-one-scenario.md)).
...
Restart-handoff tests cover a keyless assembled Web reconnect snapshot and a two-generation Loader test with temporary persistence and a fake native Host, including resident opt-in, exact ownership, same native identity, no teardown or duplicate input, generation fencing, and ordinary release.

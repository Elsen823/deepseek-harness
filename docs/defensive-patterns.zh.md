...
这些错误类别规则用于避免已经发生或险些发生的缺陷。编写生命周期、并发、子进程或清理代码前请先阅读。测试层的对应内容（真实入口、外部状态验证、资源所有权）见 [testing.md](testing.zh.md)。
...
重启交接不是释放：显式 restart intent 会限制已准入工作、运行中的 turn 与持久化 flush；超时拒绝交接并让旧代际继续服务。绝不能回退到释放，因为它会发出释放事实并丢失重新挂接所需的所有权。

...
本文说明测试层级与保持绿色结果有意义的规则。命令见根目录 [AGENTS.md](../AGENTS.md)；相关 Agent Note 承载动机。

## 层级

- **单元测试**（`pnpm run test`）：Vitest 运行包／示例的 `tests/**` 与仓库的 `scripts/**/*.spec.ts`，测试文件靠近其代码。每个注册表都测试释放清理。优先覆盖边界、错误、顺序、并发和约定回归（见 `packages/core/agent-loop/tests/contract-regressions.spec.ts`）。
- **覆盖率门禁**（`pnpm run test:coverage`）：CI 要求 `packages/*/*/src` 按文件 100%；未覆盖行可能是死代码，行覆盖率只证明执行而不证明行为。`packages/shell/pwsh-local/src` 需要真实 `pwsh`；缺少时测试跳过且 `vitest.config.ts` 豁免该文件，CI 有 `pwsh` 时仍执行 100% 门禁。
...
- **快照**（`pnpm run test:snapshot`）：无密钥预期输出覆盖传输、呈现和组装后的持久化行为。ACP 回放真实自动化服务器示例并比较归一化 JSON-RPC／日志（[ACP 快照 Agent Note](../.agents/notes/implemented/testing/2026-06-19-acp-snapshot-tests.zh.md)）；headless 通过 JSONL driver 启动显式组装，`apps/cli` 负责 `dsh --profile headless` 验收。模型 transcript 变化用 `record`，回放输入有效用 `refresh`；请审查所有差异。`text-turn` 固定完整系统提示词／工具 schema，其他 fixture 将其 token 化（[pinned-header Agent Note](../.agents/notes/archived/testing/2026-07-06-pin-request-header-content-in-one-scenario.md)）。
...
重启交接测试覆盖无密钥 assembled Web 重连快照，以及使用临时持久化和伪原生 Host 的两代 Loader 测试，包含常驻选择、精确所有权、兼容时相同原生身份、无释放或重复输入、代际隔离和普通释放。

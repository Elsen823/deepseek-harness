...
## Durable timing projection

When `ctx.sessionProjections` is available, `dsh-subagent` registers the client-safe `subagentTiming` value. Each descriptor resets the projection, `settledMs` accumulates completed post-descriptor turn spans, and `lastActivityAt` advances monotonically across the descriptor and turn events. An unfinished turn carries same-cut `active.since` and `active.through` bounds; consumers use `through` as the recorded upper bound rather than combining the projection with newer session metadata. These fields measure recorded activity and never imply an outcome, completion, or resumability.

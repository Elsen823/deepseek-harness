...
Logged, per-agent plan collaboration state for the built-in `dsh` Agent Driver, with deployment-owned guidance, direct `/plan [message]` entry and `/plan off` exit commands, and the reviewed `exit_plan_mode` exit. Alternate Drivers keep their native planning lifecycle separate: `get()` reports inactive, `set()` fails, and no `plan:policy` section is assembled. Plan mode is soft guidance; sandbox mode and approval policy enforce restrictions independently and do not read or write plan state.

## Durable state

`plan/mode` (`{ active: boolean }`) is a log-only, whole-value-replace `SessionEventMap` member. `foldPlanMode(events)` returns the last logged value or `false`, so resume, same-Driver fork, and compaction recover plan state directly from the session log. Cross-Driver forks omit the control event because ownership changes. UIs observe committed flips through `session/event`.

...
The scoped-registration surface: `Agent.ctx` is the agent's scope context (`dsh-scope`, key = the agent) — register tools/sections/variables/listeners through it for that agent alone, all unwound on disposal. `agentEvents(ctx, agent)` is the fused dispatcher for ordinary agent-subject operations (carrier + injected subject in one move); its notification mode invokes every listener and contains both synchronous throws and returned-promise rejections. The registry lifecycle pair reuses one stable routing carrier. `assembleContextFor(agent)` builds the per-agent assembly context (`agent` + `scope` together). Every Agent Driver constructs `agent.modelSelection` for its Session; the owner carries one `{ provider, model, reasoningEffort? }` selection, durable accepted intent, effective evidence, inherited default, and frozen Turn selection. Service tier and other request or Driver settings are not part of it. `createModelSelectionOwner()` is the constructor seam for Driver implementations. `CreateAgentOptions.setup(agentCtx)` and `ResumeAgentOptions.setup(agentCtx)` compose a fresh or resumed agent's scoped world while both objects remain unpublished. Setup is trusted, composition-only same-process code: drive the agent only after creation resolves.
...
The registry rejects a prepared Agent that does not supply its Session-local owner before setup or publication. A Driver that accepts a selection calls `agent.modelSelection.accept()`; acceptance is serialized with Turn-start capture, `beginTurn()` materializes an inherited default only when the first prompt is admitted, and `Session` keeps accepted intent separate from effective request evidence. The built-in `dsh` Driver captures one frozen resolved value before prompt assembly and uses it for every step and retry in that Turn; steering remains on that value, while a new accepted value waits for the next Turn.
...
#### Agent Driver registry and creation

`AgentDriver` is the named execution seam behind `ctx.agents`. Each effect-scoped registration contributes immutable `{ id, name }` discovery information and one `prepare(session, agentOptions, signal)` operation. Preparation returns an unpublished `Agent` plus narrow `start()` and execution-quiescence `dispose()` hooks; Drivers do not publish or detach registry entries. `listDrivers()` sorts by durable Driver id, `getDriver()` returns the frozen discovery record, and duplicate ids fail at registration.

- `ctx.agents.registerDriver(driver: AgentDriver): () => void` — register one Driver generation. Provider unload aborts pending preparation and drains every live Agent made by that generation before the registration effect settles.
- `ctx.agents.create(options: CreateAgentOptions): Promise<AgentHandle>` — resolve `options.driverId` or configured `defaultDriverId` (`dsh` by default), write the resolved id into the new Session header, prepare the Driver and optional setup while unpublished, then commit and publish through the final Session and Agent collision checks.
- `ctx.agents.resume(options: ResumeAgentOptions): Promise<AgentHandle>` — call persistence preparation exactly once, read the immutable Driver binding from the restored Session header, require that registered Driver, then use the same unpublished setup and publication transaction. Current defaults never override stored selection.

Both paths enter Session and Agent before announcing `session/created`, `agent/created`, then `agent/session-start`; setup or commit failure, cancellation, owner unload, Driver unload, and synchronous publication veto converge on one rollback. Concurrent same-id operations may both prepare, but final entry lets exactly one publish and retires every loser privately.

`AgentHandle = { agent: Agent; dispose(): Promise<void> }`. The caller fiber, exact Driver-provider generation, and handle co-own one memoized teardown. It aborts preparation, awaits the Driver hook that stops, reaches quiescence, and unwinds `agent.ctx`, then detaches Agent and Session; registry observers holding only a bare Agent cannot dispose it.

`ctx.agents.restartHandoff({ generation?, signal? })` is a distinct, explicit process-replacement lifecycle. When configured, it closes new admissions, waits for resident Agents and their Session flushes within `quiescenceTimeoutMs`, and atomically publishes sidecar records for the next generation; timeout or Driver refusal restores active service without calling `dispose()` or emitting teardown events. Driver validation is complete before publication, and each post-publication commit is required to be an infallible local state flip; a violating throw leaves the registry committed instead of reopening the old generation. `adoptRestartHandoffs()` claims only exact resident records, validates the persisted Session prefix and Driver binding, and transfers each newly prepared handle to its next-generation owner; if transfer succeeds before durable completion races, the replacement and its claim remain fenced. See the [process-generation handoff decision](../../../.agents/notes/implemented/architecture/2026-08-26-process-generation-restart-handoff.md).
...
- Subagent delegation is not an `Agent` method; providers create or drive ordinary handles through the Driver registry API, so delegation transports stay outside the core agent interface.
...
- **`agent/session-start` cannot gate startup** — it remains a synchronous, veto-less notification; async composition that must finish before publication belongs in the registry's `setup(agentCtx)` transaction instead.

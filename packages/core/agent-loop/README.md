...
Configured agents start automatically. Every Agent carries a Session-local Model Selection; a model call requires its provider and model, and an omitted route fails before prompt assembly or model I/O. An optional positive `maxTokens` seeds each conversation request's output cap and is logged in its request header. `maxParallelToolCalls` bounds every agent's rolling pool for parallel-safe calls and defaults to `10`; it is also the whole of the `agent-loop` Settings section, so a user layer over this entry caps the next tool group without a restart, and a value that is not a positive integer is refused at the write rather than at that group. `agents` is deliberately absent from that section — it is consumed once when the service starts, so a stored change could only look like it had an effect. `cwd` applies only to fresh sessions, while `resumeSessionId` retains persisted metadata. Configured agents use the deployment persona, and programmatic setup can shadow it per agent. This plugin supplies the per-Turn `provider` and `model` prompt variables plus the `cwd` variable; harness identity and deployment persona belong to `dsh-system-prompt`.
...
The built-in DSH Driver captures one immutable Model Selection after the first prompt is admitted and before prompt assembly. It uses that provider/model and resolved effort for every step and retry in the Turn, so a selection accepted while the Turn is running affects the next Turn and steering keeps the current value. The selected intent is appended before the first request; the effective request header remains separate and is retained when a prepared stream later fails or is cancelled. If the selection omits effort, the exact model's current provider default is resolved once at Turn start; an unavailable route or unsupported effort fails before model I/O without mutating the selected intent.

After `agent/request` returns a call config, the loop asks `ctx.llm.prepareCall()` to validate adapter-owned fields and materialize configured output-token defaults under the active turn signal. The prepared call retains the exact adapter registration across this asynchronous resolution, `request/header` logging, and terminal dispatch, so HMR cannot mix one adapter's capability result with another adapter's request. The header records the effective config and which fields came from the adapter. Before each request, the frozen Turn Model Selection restores its provider, model, and reasoning effort while request middleware retains ownership of unrelated envelope fields. A route with no registered adapter fails before model I/O; a new loop instance resolves the next Turn from its Agent owner.

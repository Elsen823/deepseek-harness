...
Both modes use bounded exponential backoff with symmetric jitter. A valid `providerRetryAfterMs` at or below `maxDelayMs` replaces local backoff without jitter. An over-cap provider delay makes normal mode delegate, while always mode uses its configured local backoff so it cannot terminate on that instruction. Deep non-loop consumers can execute the same captured policy with `nextCapturedRetry()` and `waitForRetryDelay()` after `ctx.llm.prepareCall()`; they retain attempt counting, durable facts, and cancellation ownership instead of inventing another retry configuration.
...
- **The helper is not automatic recovery** — a direct consumer must prepare once, count attempts under that captured policy, log each attempt before waiting, and discard failed partial output before retrying.

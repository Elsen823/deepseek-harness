...
- **Service tiers are unsupported** — the direct DeepSeek chat-completions wire has no adapter-owned mapping, so an explicit tier fails before network I/O.

...
- **Resolve call config** — an explicit or configured reasoning effort is checked against the exact model before any provider I/O, an adapter-configured output cap is materialized when the request omits one, and an opaque service tier reaches the selected adapter for support validation and wire mapping.
...
- **Prepared attempts are one-shot** — each prepared handle can be dispatched exactly once, its call-config fields must match the prepared config, and `nextAttempt()` retains the same adapter generation and resolved facts.

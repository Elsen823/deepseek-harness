...
- **Only the priority service tier is supported on Responses** — `openai-responses` and `openai-codex-responses` send `priority` as `service_tier`; every other value or protocol fails before network I/O instead of silently changing or ignoring the request.

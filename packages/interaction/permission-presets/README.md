...
User-facing permission presets for the built-in `dsh` Agent Driver through `ctx.permissionPresets` ([`PermissionPresetService`](src/index.ts)). Each configured name bundles `sandbox/mode` with `approval/policy`; the defaults are `workspace-write` (`workspace-write` + `ask`) and `danger-full-access` (`danger-full-access` + `never`). UI adapters may expose the table as one selector, while sandbox execution and approval continue to consume their own knobs. Alternate Drivers keep their native permission controls and reject this service's write path.
...
The service owns the `permissionPresets` Settings namespace. Its `defaultPreset` is the default for future sessions: the composition entry uses `Config.defaultPreset`, or infers the preset matching the composed sandbox and approval defaults when omitted. A committed Settings change is read when the next built-in `dsh` Session is created; creation pins `permission/preset`, `sandbox/mode`, and `approval/policy` into that Session, so later changes never alter it. A resumed `dsh` seed, including an explicitly empty one marked by `session/end-seed`, preserves its effective permission and receives only missing durable facts rather than the latest user default. Mounting the service also sweeps already-live `dsh` Sessions, so an HMR replacement pins any owned Session created while the plugin was absent. Alternate Driver Sessions receive none of these events.

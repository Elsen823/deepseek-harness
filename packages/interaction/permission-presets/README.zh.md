...
通过 `ctx.permissionPresets`（[`PermissionPresetService`](src/index.ts)）为内置 `dsh` Agent Driver 提供面向用户的权限预设。每个配置名称都会将 `sandbox/mode` 与 `approval/policy` 组成一组；默认项为 `workspace-write`（`workspace-write` + `ask`）和 `danger-full-access`（`danger-full-access` + `never`）。UI 适配器可以将该表作为单个选择器公开，而沙箱执行与审批仍分别消费各自的调节项。备用 Driver 保留其原生权限控制，并拒绝本服务的写入路径。
...
该服务拥有 `permissionPresets` Settings namespace。其 `defaultPreset` 是未来会话的默认值：组合项使用 `Config.defaultPreset`；省略时，则推断与组合后的沙箱和审批默认值匹配的 preset。已提交的 Settings 变更会在下一个内置 `dsh` Session 创建时读取；创建过程将 `permission/preset`、`sandbox/mode` 和 `approval/policy` 固定到该 Session 中，因此后续变更绝不会改变它。恢复的 `dsh` seed，包括由 `session/end-seed` 标记的显式空 seed，都会保留其有效权限，只补齐缺失的持久事实，而不会采用最新的用户默认值。挂载服务时还会遍历所有已存活的 `dsh` Session，因此 HMR（热模块替换）会固定插件缺席期间创建的所有自有 Session。备用 Driver Session 不会收到这些事件。

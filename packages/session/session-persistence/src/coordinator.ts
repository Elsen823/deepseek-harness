 ...
 * unknown to the active plugin composition and not marked ignorable. Distinct
 * from {@link SessionPersistenceCorruptionError} — nothing is damaged; the raw
 * log remains readable at {@link location} when the backend keeps one artifact
 * per session.
 */
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  /**
   * Refuse a required event type absent from the active composition. An
   * ignorable event may be read without its declaring plugin because its writer
   * asserted that omission cannot change reconstruction. Runs on normalized
   * events after supported legacy conversions.
   */
  private assertEventsSupported(meta: SessionHeader, events: readonly SessionEvent[]): void {
    for (const event of events) {
      if (this.ctx.sessions.isEventTypeRegistered(event.type) || event.ignorable === true) continue
      throw this.unsupported(meta, `session "${meta.id}" contains required event type "${event.type}" (seq ${event.seq}) that is not registered by the active plugin composition; refusing to interpret the log`)
    }
  }

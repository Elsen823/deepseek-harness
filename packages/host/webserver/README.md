...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. Route owners that buffer a complete GET/HEAD body use the exported `writeHttpBody` helper for gzip/Brotli, ETag/304, and the `isDirectLoopback` check ([origin encoding and cache](../../../.agents/notes/implemented/architecture/2026-08-21-web-origin-encoding-and-cache.md)). The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). This server serves browsers only; Electron loads dist over `file://` and carries fetch over an IPC bridge. This package never prints; the URL line belongs to the shell.

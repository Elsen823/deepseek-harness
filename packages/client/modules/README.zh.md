...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希和包专属 `dsh.client.external` 请求写入启动图，把动态提供方排在消费者之前，并通过 `/plugins` 提供该文件（仅当单个 `rev` 同时匹配当前图行和刚读出的字节时不可变；source map 仅直连回环；当 Accept-Encoding 允许有益的表示时协商 gzip／Brotli — [源站编码与缓存](../../../.agents/notes/implemented/architecture/2026-08-21-web-origin-encoding-and-cache.zh.md)）。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。

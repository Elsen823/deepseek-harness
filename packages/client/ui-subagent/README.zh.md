...
Host 会注册 `ui-subagent` 设置命名空间，默认值为 `hideInactive: false` 与 `inactiveAfterMinutes: 60`；阈值可设为 1–35,791 分钟，即便携式定时器延迟可表示的最大整分钟数，而“通用设置”页会提供开关。启用后，浏览器会在非运行行的持久化 `subagentTiming.lastActivityAt` 达到该时长时将其隐藏。缺少该投影的旧检查点会在最新提示词或创建时间与累计 LLM／工具运行耗时同时存在时使用二者之和；缺少这两类计时依据的行会继续显示。运行中行、诊断行，以及通往可见后代所需的旧祖先也会继续显示。每个已挂载的目录控件只会调度最近的过期截止时间，因此即使没有新的 Host 帧，条目也能按时消失。运行时仍保留完整目录、地址、会话、记录与继续执行状态；过滤绝不会中断、删除、归档或关闭已打开的 child。
...
- **过期判断会比较浏览器时间与 Host 事件时间**：远程设备时钟偏差较大时，阈值到达时间会偏移。旧版提示词或创建时间／运行耗时回退值属于估算，不包含排队或等待间隔；新检查点使用精确活动计时。同时缺少这两类依据的行会按保守策略继续显示。

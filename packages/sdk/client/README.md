...
The subprocess starts lazily on first use and stays owned by the instance across `run()` calls; `close()` (or `await using`) is required so the child is always reaped. `start()` memoizes the `initialize` handshake: the absolute workspace cwd, provider/model route, optional positive `maxTokens` output cap, and optional default `driverId`. A failed handshake reaps the runtime and swaps in a fresh client, so a later call retries with a new subprocess until terminal `close()`. The cap applies to each root-agent request and is inherited by in-process descendants; compaction plugins own their separate summary limits. `drivers()` reads the active catalog, `runtime(sessionId)` reads one process-local current value and materializes `cold` for a persisted durable Session, and `session(id?, driverId?)` opens a named or fresh handle with an immutable lazy-creation Driver selection.

`run(input, { sessionId?, driverId?, onNotification? })` owns one activity interval: it queues the prompt, waits until its `MessageId` appears in a durable `agent/inbox/spliced` receipt, then collects through the next whole-agent `idle`. `driverId` selects lazy creation and rejects if the named Session is already bound to another Driver; changing Drivers requires a new Session or explicit fork outside this SDK protocol. The method returns `RunResult { sessionId, finalResponse, events, notifications }`. `finalResponse` is the last committed root-session assistant text in that interval, not a response causally assigned to the prompt; steering, injected context, and other queued work may contribute before idle. `events` contains root-session events, while `notifications` also contains descendants discovered from `subagent.started`, all in wire order. The result carries no prompt-level status or turn reason. Transport loss, timeout, and protocol violations reject; model outcomes remain observable in the event stream without being attributed to one input.

`selectedModelSelection(events)` folds the latest durable `model/selected` intent from a returned event interval. It keeps accepted provider/model/effort intent distinct from effective request evidence in `request/header` or `agent-driver/model-request`, so a selection accepted before its first request remains observable without being mistaken for a completed model call.

## HarnessClient

The protocol client under the owned-run API: explicit `start()`/`initialize()`/`drivers()`/`runtime()`/`prompt()`/`request()`/`close()`, plus notification subscriptions. `prompt()` returns the queued message id as soon as the runtime accepts it; it never waits for agent activity. `subscribe(filter?)` returns a `NotificationSubscription` (awaitable `next()`, non-blocking `tryNext()`, async iteration); `subscribeSessionTree(id)` scopes to one session and descendants discovered from `subagent.started`, including nested `session.runtime.status.sessionId` notifications. Error types are exported from this package: `JsonRpcResponseError` (wire error response, code/data preserved), `RequestTimeoutError` (a configured bound elapsed), `SdkProtocolError` (a response outside the documented protocol), and `TransportClosedError` (the runtime is gone; the message carries the exit code and a bounded stderr tail).

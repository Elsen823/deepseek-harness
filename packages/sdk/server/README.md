...
`initialize` is the runtime-readiness boundary: when the server is mounted by a Loader composition, it waits for the current plugin tree to settle before replying, so async sibling capabilities such as initial MCP tool discovery are visible to the first prompt. Hand-built contexts without Loader remain immediately usable. The response carries the wire-stable server name and active Agent Driver catalog. Optional `initialize.driverId` selects the default immutable binding for SDK-created Sessions; an unknown Driver rejects initialization. Optional positive `initialize.maxTokens` becomes the request output cap of each SDK-created agent and its in-process descendants; invalid values reject initialization, while omission allows the selected adapter or provider route default to apply. `agent/drivers` rereads the active catalog, and `session/runtime` returns one process-local current value or `null` without activating the Session. `session/prompt` accepts an optional lazy-creation `driverId`, rejects a mismatch with an existing Session, queues one identified user message, and immediately returns `{ messageId }`. The server emits `session.created`, streams every durable fact as `session.event`, preserves binary Agent transitions as `session.status`, and emits richer transient state as `session.runtime`; it does not assign an assistant message or `turn/end` to the prompt. Independent requests may enqueue more work on the same Session. Persistence roots and persona come from `cordis.yml`.

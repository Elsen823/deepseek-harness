...
Every created or resumed Agent exposes its Session-local `ModelSelection` owner. A Session accepts and durably records `model/selected` before any request consumes it; the Agent Driver resolves one immutable value per Turn, and prompt assembly does not route selection. The [Agent-owned Turn Model Selection](../architecture/2026-08-25-agent-owned-turn-model-selection.md) owns the shared lifecycle and effective evidence distinction.
...
The session header identity presentation is owned separately by the [Web model-selection identity note](2026-08-25-web-model-selection-identities.md). It consumes the durable projection without changing the directory's selection or the Models settings default.

...
host 侧 `session.create({ workspaceId, driverId? })` 一体产出 Session、Agent、cwd 与不可变 Driver 绑定（作为不可拆分的原子整体）；client 侧就是这次出生的镜像——会话行进入 list mirror 的瞬间，client 为它铸 Agent scope（actx + provide + 输入面全套挂上）：
...
- 实体化时点 = 用户选定 Workspace（cwd 确定）的瞬间：client 当场调 `session.create({ workspaceId, driverId? })`，一并携带 New Session Hero 中暂存的 Driver，并拿到完整实体。
...
`workspaces.connectWorkspace(workspaceId, options?): Promise<SessionId>`（归属 WorkspaceRuntime——它同时持有 Workspace 规范 path、Session 摘要与可选的创建期 Driver 选择）：

- 复用臂：list mirror 中找 `blank && cwd == workspace.path && sessionIds.includes(id)`——host 自己的成员规则，绝不只按 cwd——并在已选择 Driver 时要求 `summary.driverId === options.driverId`。没有账户槽位的 cwd 匹配（CLI（命令行界面）/TUI 在 host cwd 创建的会话，或已删除/重建的注册）会打开一个任何分组界面都无法显示在该 Workspace 下的会话，因此落到新建臂（见[成员复用修复](../bug-fix/2026-08-05-workspace-blank-session-reuse-membership.zh.md)）；Driver 不匹配同样会落到新建臂，因为 Session 绑定不可变（[Driver/Workspace 决策](../bug-fix/2026-08-24-new-session-driver-workspace-pair.zh.md)）。命中直接返回该 id，不新建。
- 新建臂：未命中则 `session.create({ workspaceId, driverId? })`，返回新 id。
...
- blank Hero 中改选 Workspace 也会把暂存或当前 blank Session 的 Driver 传给 `connectWorkspace`；若目标 id 与当前 id 不同，先把当前 input machine 的非空 draft 搬到目标 scope，再 `sessions.open(nextId)`。旧 blank 实体不删除，只因不再 current 而从列表隐藏。
...
- `session.create` 请求结构：workspaceId/cwd 二选一，再加可选的调用方预分配 sessionId 与不可变 driverId（同 id 同 cwd 重试幂等，cwd 不同时报 `session-conflict`，Driver 不同时报 `agent-driver-conflict`）。
